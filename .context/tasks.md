...
## Recurring work-unit check

- At major work-unit transitions, including To Do reviews, check [Amnezia #2933](https://github.com/amnezia-vpn/amnezia-client/issues/2933) and linked fixes, then [published releases](https://github.com/amnezia-vpn/amnezia-client/releases) for inclusion in an available **macOS Amnezia client** build. Record check date, issue/fix status, and the first applicable released version (plus any macOS OS prerequisite). Issue closure or a merged patch alone is insufficient. Continue until a fixed Mac build passes our coordinated trial; no background monitor is requested. Last review: 2026-09-13, issue open, no matching fix or fixed Mac release identified; installed 5.0.1.5. See [revisit criteria](../docs/mac-client-stability.md#amnezia-revisit-check).

## Next work

- id: 01M2D00GJ5HE1591CYW6KSFVV1 — title: Resolve recurring Mac VPN sleep/wake crashes — owner: Codex / Anthony — status: OneXraySE regional routing/awake connectivity pass; repeated sleep/wake validation pending — last update: 2026-09-13
  - [Final comparison and decision triggers](../docs/mac-client-stability.md#final-client-selection-notes): Anthony installed OneXraySE through Homebrew for REALITY-only scope. Both original regional identities pass native checks; intermittent stability is not established yet. Keep Streisand as the immediate alternative and v2rayN if consolidating both protocols becomes the priority. Anthony accepts Streisand tracking.
  - Preserve the server/image, existing identity and AWG in Amnezia; iPhone and Cape Town remain unchanged. SFM is removed after the demonstrated REALITY version mismatch; Happ is withdrawn over provider-driven remote configuration. Do not reopen those decisions merely to avoid a client compatibility problem.
  - Three watchdog/panic pairs strongly implicate the Amnezia/macOS network interaction and contain its Xray-only local path; exact lock cycle/selected-profile state remain unresolved. No AWG-only sleep panic identified. Startup-item cleanup followed the latest reboot. Detailed evidence and private artifact paths live in the diagnosis/handoff.
  - Retire raw Amnezia daemon-socket polling/deactivation: a separate cleanup-related service SIGSEGV makes the old watchdog unsuitable as trusted recovery. Do not rerun its ignored scripts.
  - Next validation: practical use and owner-coordinated repeated sleep/wake on battery/AC with new-report inspection. Awake regional exits/HTTPS, fresh DNS, encrypted-resolver reachability and clean disconnect/reconnect passed. IPv6 is unavailable even disconnected; verify prevention on a capable network later. Keep one active tunnel; no forced sleep. One passing wake does not resolve an intermittent crash.
  - Stop broad comparison once a candidate works. Anthony rejects local Amnezia patching/fork maintenance and requests upstream checks between major work units (recurring check above). Pause routine Mac Amnezia REALITY use pending a fixed-build trial; awake reproductions mean avoiding sleep is insufficient. Keep AWG scope unchanged. Server architecture work resumes separately and does not itself resolve client/OS stability.
  - [OneXraySE configuration](../docs/mac-client-stability.md#onexrayse-regional-configuration): owner approved and saved All via VPN/full capture/default system exceptions/DoT, with automatic connection off. Stockholm → Cape Town → Stockholm all passed; final Stockholm selected/disconnected. Accidental macOS denial resolved by retry/Allow. No reboot/reset/cloud change. The [4/5 Amnezia patch estimate](../docs/mac-client-stability.md#amnezia-patch-feasibility) is historical.
