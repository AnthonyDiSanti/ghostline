...
Implementation update (2026-09-10): the later Stockholm work implemented tracked-EIP park/redeploy and scoped full release; see [current lifecycle commands](development.md#on-demand-regional-lifecycle). The later [ECS trial](ecs.md) also implements explicit start/stop and automatic ECR/Parameter Store restoration. Idle expiration and a remote controller remain unimplemented. The management recommendations below describe the original assessment; regional rates are dated Cape Town figures.
...
4. Preserve CloudFormation ownership of EIPs while removing disposable compute. The subsequently implemented parked stack keeps the EIP logical IDs and allocations tracked; `destroy` deliberately releases them. A naive stack deletion with Retain followed by a fresh deploy does not automatically adopt orphaned allocations. Runtime restoration remains a separate operation; avoid manual termination as an untracked CloudFormation lifecycle mechanism.
