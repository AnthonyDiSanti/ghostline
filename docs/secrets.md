...
Updated 2026-09-10. The six Stockholm server/device values are now imported into eu-north-1 Standard SecureStrings and verified byte-for-byte for the [ECS trial](ecs.md). Existing host files, local recovery copies and device identities remain unchanged. Cape Town normalization/import remains separate. VPN guest access uses protocol credentials, not SSH accounts or keys; the ECS host requires no administrator SSH key.
...
| Operator SSH private key per deployment | Administrator access from `.local/keys/<resourceName>` | Protected operator machine and owner-mediated LastPass recovery; not fetched by the host |
